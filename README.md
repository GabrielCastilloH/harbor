...
Harbor uses **Firebase Authentication** with email/password and email verification. The authentication system is designed to be simple and secure, using Firebase's default recommended methods without overcomplicating the process.

### Authentication States

The app manages four distinct authentication states:

1. **`unauthenticated`** - User is not signed in
2. **`unverified`** - User is signed in but email is not verified
3. **`no-profile`** - User is signed in and email is verified, but no profile exists in Firestore
4. **`authenticated`** - User is signed in, email is verified, and profile exists in Firestore

### Flow Logic

```
User Action → Firebase Auth State → AppContext Check → Navigation Decision
```

#### Account Creation Flow:

1. **Create Account** → Firebase creates user → Send verification email → Sign out user
2. **Email Verification Screen** → User clicks verification link → Firebase verifies email
3. **Auth State Change** → AppContext detects verification → Check Firestore profile
4. **Navigation** → If no profile → AccountSetupScreen, If profile exists → Main App

#### Sign In Flow:

1. **Sign In** → Firebase authenticates user → Check email verification status
2. **If email not verified** → EmailVerificationScreen
3. **If email verified** → Check Firestore profile
4. **Navigation** → If no profile → AccountSetupScreen, If profile exists → Main App

### Firebase Default Methods Used
...
- **`user.reload()`** - Refresh user data to get latest verification status
- **`onAuthStateChanged()`** - Listen for authentication state changes
- **`sendPasswordResetEmail()`** - Reset password functionality
...
switch (authState) {
  ...
  case "no-profile":
    return <AccountSetupScreen />; // Profile setup
  case "unverified":
    return <AuthNavigator />; // Email verification
  case "unauthenticated":
    return <AuthNavigator />; // Sign in/Create account
}
```

#### Email Verification Process

1. **Account Creation**: User creates account → Firebase sends verification email → User signed out
2. **Verification**: User clicks email link → Firebase verifies email → Auth state changes
3. **Profile Check**: AppContext checks Firestore for user profile
4. **Navigation**: Based on profile existence, user is taken to appropriate screen

### Security Features

- **Cornell Email Validation**: Only @cornell.edu emails allowed
- **Email Alias Prevention**: Rejects emails with `+` symbols to prevent multiple accounts
- **Email Verification Required**: Users cannot access app without verified email
- **Profile Completion Required**: Users cannot access main app without complete profile
